...
protected:
  ...
  bool verify_code_impl(It begin, It end, It code_loc) noexcept {
    ...
    crc = calculate_code_impl_(begin, code_loc, crc);
    crc = calculate_code_impl_(code_loc + sizeof(uint16_t), end, crc);
    return crc == ((*code_loc << 8) | (*(code_loc + 1)));
  }

  template <typename It>
  uint16_t calculate_code_impl_(It begin, It end,
                                uint16_t crc = 0xFFFF) noexcept {
    It body_iter = begin;
    while (body_iter != end) {
      crc = (crc >> 8) ^ crc16_table[(crc ^ *body_iter++) & 0xFF];
    }
    return crc;
  }
...
#endif // COMPONENTS_VERIFICATION_ALGORITHM_HPP
